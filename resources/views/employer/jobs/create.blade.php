            ...
            <form action="{{ route('jobs.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <header class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-300 mt-1">Fill the details below to publish a new job.
                        </p>
                    </div>
                </header>
                ...
                @if ($errors->any())
                    <div
                        class="rounded-md bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 p-3 text-sm text-red-700 dark:text-red-300">
                ...
                {{-- Description --}}
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Job
                        Description</label>
                    <textarea id="description" name="description" rows="5"
                        placeholder="Role summary, responsibilities, expectations..." required
                ...
                <div>
                    <label for="responsibilities"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Responsibilities</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="category_id" name="category_id"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div x-data="dropdownAutocomplete()" class="relative w-full">
                        ...
                        <!-- Input field -->
                        <input id="location" name="location" type="text" x-model="query" @focus="open = true"
                            @click.outside="open = false" placeholder="Select or type a location..."
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 shadow-sm px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500"
                            autocomplete="off">

                        <!-- Dropdown list -->
                        <ul x-show="open && filteredLocations.length > 0"
                    ...
                    {{-- Work type --}}
                    <div>
                        <label for="work_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Work
                            Type</label>
                    ...
                    <div>
                        <label for="application_deadline"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Application Deadline</label>
                ...
                <div x-data="tagInputs(@json(old('skills', [])), @json(old('qualifications', [])), @json(old('technologies', [])))" class="space-y-4">
                    ...
                    <div x-data="dropdownTagsSkills({{ json_encode(old('skills', [])) }})" class="space-y-2">
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            <template x-for="(tag, index) in selected" :key="index">
                                <span
                                    class="inline-flex items-center gap-2 px-3 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 rounded-full text-sm">
                                    <span x-text="tag"></span>
                                    <button type="button" @click="removeSkill(index)"
                                        class="text-indigo-500 hover:text-indigo-700">&times;</button>
                    ...
                    <div x-data="dropdownTagsQualifications({{ json_encode(old('qualifications', [])) }})" class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <div x-show="open" @click.away="open = false"
                                ...
                                <template x-for="qual in allOptions" :key="qual">
                                    <button type="button" @click="addTag(qual)"
                                        class="w-full text-left px-3 py-2 text-sm hover:bg-indigo-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-200">
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            <template x-for="(tag, index) in selected" :key="index">
                                <span
                                    class="inline-flex items-center gap-2 px-3 py-1 bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-300 rounded-full text-sm">
                                    <span x-text="tag"></span>
                    ...
                    <div x-data="dropdownTagsTech({{ json_encode(old('technologies', [])) }})" class="space-y-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Technologies
                            (tags)</label>
                        ...
                        <div class="relative">
                            ...
                            <div x-show="open" @click.away="open = false"
                                ...
                                <template x-for="tech in allOptions" :key="tech">
                                    <button type="button" @click="addTag(tech)"
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            <template x-for="(tag, index) in selected" :key="index">
                                <span
                                    class="inline-flex items-center gap-2 px-3 py-1 bg-sky-50 dark:bg-sky-900 text-sky-700 dark:text-sky-300 rounded-full text-sm">
                                    <span x-text="tag"></span>
                                    <button type="button" @click="removeTag(index)"
                                        class="text-sky-500 hover:text-sky-700">&times;</button>
                    ...
                    <div x-data="dropdownTagsBenefits(@json(old('benefits', [])))" class="space-y-2">
                        ...
                        <div class="relative" x-cloak>
                            ...
                            <div x-show="open" @click.away="open = false" x-transition
                                ...
                                <template x-for="benefit in allOptions" :key="benefit">
                                    <button type="button" @click="addTag(benefit)"
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            <template x-for="(tag, index) in selected" :key="index">
                                <span
                                    class="inline-flex items-center gap-2 px-3 py-1 bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300 rounded-full text-sm">
                                    <span x-text="tag"></span>
                                    <button type="button" @click="removeTag(index)"
                                        class="text-purple-500 hover:text-purple-700">&times;</button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="salary_min" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Salary
                            Min</label>
                        <input id="salary_min" name="salary_min" type="number" step="0.01"
                            value="{{ old('salary_min') }}"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 px-3 py-2"
                    ...
                    <div>
                        <label for="salary_max" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Salary
                            Max</label>
                        <input id="salary_max" name="salary_max" type="number" step="0.01"
                ...
                <div x-data="imagePreview()" class="space-y-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Company Logo / Branding
                        (optional)</label>

                    <div class="flex items-center gap-4">
                        <div
                            class="w-24 h-24 bg-gray-50 dark:bg-slate-700 rounded-md border flex items-center justify-center overflow-hidden">
                        ...
                        <div class="flex-1">
                            <input id="branding_image" name="branding_image" type="file" accept="image/*"
                                @change="onFileChange($event)" class="block w-full text-sm text-gray-700">
                            <p class="text-xs text-gray-400 mt-1">Recommended: PNG/JPG up to 2MB.</p>
                            <div class="mt-2 flex gap-2">
                                <button type="button" @click="remove()"
                                    class="px-3 py-1 bg-red-50 text-red-700 rounded text-sm hidden"
                                    x-ref="removeBtn">Remove</button>
                            </div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('jobs.index') }}"
                        class="inline-flex items-center px-4 py-2 bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-gray-200 rounded hover:bg-gray-300 transition">Cancel</a>
                    <button type="submit"
                        class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md shadow-sm transition">Create
                        Job</button>
                </div>
            </form>
        </div>
